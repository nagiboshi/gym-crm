...
export class PurchaseVoucherController {
  private readonly logger = new Logger(PurchaseVoucherController.name);
  ...
  async createOne(
    ...
    const inventoryList = purchaseVoucher.items.map( pVoucher => new InventoryItem(pVoucher));
    const savedInventoryList = await this.inventoryService.repo.save(inventoryList);
